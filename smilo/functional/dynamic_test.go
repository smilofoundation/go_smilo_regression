...
var _ = Describe("QFS-02: Dynamic fullnodes addition/removal testing", func() {
	const (
		numberOfFullnodes = 4
	)
	var (
	...
	It("QFS-02-01: Add fullnodes", func() {
		testFullnodes := 1
	...
	It("QFS-02-04: Reduce fullnode network size below 2F+1", func() {
	...
	It("QFS-02-05: Reduce fullnode network size below 2F+1", func() {
		By("Ensure that blocks are generated by fullnodes", func() {
			tests.WaitFor(blockchain.Fullnodes(), func(geth container.Ethereum, wg *sync.WaitGroup) {
				Expect(geth.WaitForBlocks(5)).To(BeNil())
				wg.Done()
